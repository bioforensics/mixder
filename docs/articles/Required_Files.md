...
2.  **The sample manifest**  
    **NOTE** As of version 0.10, a single sample ID (along with a single
    replicate ID, if warranted), can be provided in place of a sample
    manifest by selecting the “Single Mixture” option. If “Multiple
    Mixtures” option is selected, a sample manifest is required. This
    file lists the Sample IDs of the files to run. The `SampleID` is
    extracted from the `Sample Name` field in the Sample Report. The
...
MixDeR provides global population allele frequencies for Kintelligence
