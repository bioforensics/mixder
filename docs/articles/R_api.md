...
MixDeR can be run either through the associated Shiny app or using the
below R functions.
...
**NOTE x 2**: Using the R API, there are two options to specify
samples:  
...
specified (using the `sample` and `replicate` arguments)  
...
for more information about the defaults), using the 1000 Genomes Global
...
As mentioned above, a single sample ID can be specified instead of a
sample manifest:

    run_mixder_report(sample = "Sample01", sample_reports="/path/to/sample_reports_dir/", output="output_dir_name")

To run a conditioned mixture deconvolution using the default settings
and the 1000 Genomes Global allele frequency data, conditioning on both
Ref1 and Ref2, use the following function:
...
To utilize a custom allele frequency file for both the major and minor
contributor:
...
To utilize separate allele frequency data for the major and minor
contributor:
...
To change the default settings, such as the Allele 1 and Allele 2
probability thresholds and the dynamic and static ATs:
...
calculates metrics using the default settings (See [Running Mixture
Deconvolution](https://bioforensics.github.io/mixder/articles/Running_MixDecon.md)
for more information about the defaults), using the 1000 Genomes Global
allele frequency data. Specifying the references directory, as well as
the assumed major and minor contributor, are required for this function:
...
Similar as the `run_mixder_report` function, a conditioned deconvolution
can be run:
...
Changing the range of tested allele 1 and allele 2 probability
thresholds:
...
**Full reference for this function, including default settings, [can be
found
here](https://bioforensics.github.io/mixder/reference/run_mixder_ancestry.md).**

Again, the options are similar to `run_mixder_report` and
`run_mixder_metrics` with additional required inputs: the set of SNPs to
use for PCA (either `ancestry` or `all`) and the population groups to
use for PCA (either `superpopulations` or `subpopulations`). The
following function runs an unconditioned mixture deconvolution and
performs PCA using the default settings (See [Running Mixture
Deconvolution](https://bioforensics.github.io/mixder/articles/Running_MixDecon.md)
for more information about the defaults), using the 1000 Genomes Global
allele frequency data.
...
To run a conditioned deconvolution for ancestry prediction, using only
ancestry SNPs and subpopulations for PCA:
...
Same as above, the mixture deconvolution default settings can be
changed:
