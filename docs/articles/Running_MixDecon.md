...
Mixture deconvolution utilizes [EuroForMix
(EFM)](https://github.com/oyvble/euroformix).
...
- EUR 1000G data  
- SAS 1000G data  
The user may also upload a custom file. See `Required Files` page for
more details about the format for uploading a custom AF file.
...
required for a SNP to. be included (default = 0.015 or 1.5%).  
*A quick note on the ATs: both the static and dynamic ATs are applied
and the one producing the higher AT will be used in the EFM software.*
