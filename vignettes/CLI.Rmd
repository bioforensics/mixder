...
MixDeR can be run either through the associated Shiny app or through the command line.  

There are three main commands with multiple optional arguments:  
...
**NOTE x 2**: on the command line, a sample manifest can be used (as required by the Shiny app) listing the sample ID in the first column (if applicable, the replicate ID in the second column) for multiple samples to be run consecutively, or a single sample ID (and, if applicable, a replicate ID) can be specified. Please see below for additional details.
...
__Full reference for this command, including default settings, [can be found here](../reference/run_mixder_report.html).__

The following command runs an unconditioned mixture deconvolution and creates a report using the default settings (See [Running Mixture Deconvolution](Running_MixDecon.html) for more information about the defaults), using the 1000 Genomes Global allele frequency data:  
...
To run a conditioned mixture deconvolution using the default settings and the 1000 Genomes Global allele frequency data, conditioning on both Ref1 and Ref2, use the following command:
```
...
__Full reference for this command, including default settings, [can be found here](../reference/run_mixder_metrics.html).__ 

The options are similar as the ```run_mixder_report``` function. The following command runs an unconditioned mixture deconvolution and calculates metrics using the default settings (See [Running Mixture Deconvolution](Running_MixDecon.html) for more information about the defaults), using the 1000 Genomes Global allele frequency data. Specifying the references directory, as well as the assumed major and minor contributor, are required for this command:  
...
__Full reference for this command, including default settings, [can be found here](../reference/run_mixder_ancestry.html).__

Again, the options are similar to ```run_mixder_report``` and ```run_mixder_metrics``` with additional required inputs: the set of SNPs to use for PCA (either ```ancestry``` or ```all```) and the population groups to use for PCA (either ```superpopulations``` or ```subpopulations```). The following command runs an unconditioned mixture deconvolution and performs PCA using the default settings (See [Running Mixture Deconvolution](Running_MixDecon.html) for more information about the defaults), using the 1000 Genomes Global allele frequency data.
```
