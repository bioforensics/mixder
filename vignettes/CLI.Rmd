...
The following command runs an unconditioned mixture deconvolution and creates a report using the default settings (See [Running Mixture Deconvolution](Running_MixDecon.html) for defaults), using the 1000 Genomes Global allele frequency data:  
...
The options are similar as the ```run_mixder_report``` function. The following command runs an unconditioned mixture deconvolution and calculates metrics using the default settings (See [Running Mixture Deconvolution](Running_MixDecon.html) for defaults), using the 1000 Genomes Global allele frequency data. Specifying the references directory, as well as the assumed major and minor contributor, are required for this command:  
